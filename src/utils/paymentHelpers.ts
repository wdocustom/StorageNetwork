...
const DIRECT_PRO_FEE_RATE = 0.03;  // 3%  — direct leads on Pro plan
 ...
 *   Direct lead  (source: partner_link/installer_manual) → Free plan: 15%, Pro plan: 3%
 */
function resolveFeeRate(source?: string, isPro?: boolean): { rate: number; label: string } {
  ...
  // Direct lead
  if (isPro) {
    return { rate: DIRECT_PRO_FEE_RATE, label: "Platform Fee (3%)" };
  }
