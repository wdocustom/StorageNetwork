...
import { getInstallerReviews } from "@/app/actions/reviews";
...
export default async function InstallerPortfolioPage({ params }: PageProps) {
  ...
  const reviewData = profile.show_reviews !== false
    ? await getInstallerReviews(profile.id)
    : null;
  ...
  return (
    <div className="min-h-screen bg-[#080c16]">
              ...
              <div className="flex flex-wrap items-center justify-center gap-1.5">
                ...
                {reviewData && reviewData.totalReviews > 0 && (
                  <span className="inline-flex items-center gap-1 rounded-full border border-yellow-400/20 bg-yellow-400/10 px-2.5 py-1 text-[11px] font-bold text-yellow-400">
                    ...
                    {reviewData.averageRating} &middot; {reviewData.totalReviews} Review{reviewData.totalReviews !== 1 ? "s" : ""}
                  </span>
                )}
              </div>
            </div>
      ...
      {/* ── Portfolio Showcase (Photos + Reviews Interleaved) ──────────── */}
      <PortfolioShowcase
        photos={photos}
        reviews={reviewData}
