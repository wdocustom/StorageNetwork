...
export default function RealtorJoinPageClient() {
  return (
    <Suspense>
...
function RealtorJoinPageInner() {
  ...
  return (
    <div className="min-h-screen bg-slate-950">
      ...
      {/* ── FAQ — visible mirror of the FAQPage JSON-LD on page.tsx ─────
          Real questions realtors search for ("how much should I spend",
          "are reusable totes a good closing gift", etc.) — Google now
          discounts FAQPage schema that isn't actually on the page, so
          the visible copy and the structured data must agree. */}
      <section className="border-t border-slate-800 bg-slate-950 px-6 py-20 lg:px-16">
        <div className="mx-auto max-w-3xl">
          ...
          <div className="space-y-8">
            <FaqItem
              q="What's the best closing gift for buyers right now?"
              a="The closing gifts that get remembered are the ones that arrive when your client is using them — and nobody's using a wine glass during their move. Reusable moving totes show up before the move-in date, get filled, get returned. Your name is the one they think of every time they pack a box."
            />
            <FaqItem
              q="How much should a realtor spend on a closing gift?"
              a="Most agents land between $100 and $400 depending on commission size and relationship. Storage Network packages start around $200 for a 20-tote starter and run up to $500 for a 50-tote premium move — priced per-package, no subscription, only pay when you send."
            />
            <FaqItem
              q="Are reusable moving totes actually a good closing gift?"
              a="They're a closing gift that doubles as a service. Buyers and sellers consistently rank moving as the most stressful part of the transaction; handing them 20–50 stackable, reusable totes plus a local pro who delivers and picks them up removes the worst part of the day they remember you for."
            />
            <FaqItem
              q="When should I send the gift — at signing or before move-in?"
              a="Send the gift link as soon as you've got a closing date. The recipient picks their own delivery window through the link, so the totes show up the week of the move, not the week of the signature. You get the credit; they get the help on the day they actually need it."
            />
            <FaqItem
              q="What if my client is in a city where you don't have an installer?"
              a="We confirm coverage at checkout — if no installer covers the delivery ZIP, the gift won't go through and you won't be charged. Coverage is expanding monthly, but you can always check a specific area before committing."
            />
          </div>
        </div>
      </section>
...
function FaqItem({ q, a }: { q: string; a: string }) {
  return (
    <div>
      <h3 className="mb-2 text-base font-bold text-white">{q}</h3>
