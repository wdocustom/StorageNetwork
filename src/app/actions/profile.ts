...
export async function getFullProfileBySlug(slug: string) {
  ...
  return profileCache.getOrFetch(key, async () => {
    const { data, error } = await supabase
      .from("profiles")
      .select(
        "id, first_name, last_name, business_name, trade_name, phone, city, state, avatar_url, slug, is_pro, is_partner, bio, instagram_url, facebook_url, portfolio_photos, lead_time_days, working_days, services_config, pro_trial_ends_at, stripe_subscription_id"
      )
