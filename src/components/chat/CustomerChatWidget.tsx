...
import { MessageSquare, X, Send, Loader2, ExternalLink, Mail, Sparkles, Package, ArrowUp, Layers, Flower2, ChevronRight, Mic } from "lucide-react";
import type { RackConfig, InstallerChatContext, CustomerInfo } from "@/lib/ai/customer-chat-prompt";
...
interface CustomerChatWidgetProps {
  ...
  skipWelcome?: boolean;
  /** Callback when AI collects customer info (name, address) — pre-fills Step 4 form */
...
function parseCustomerInfo(text: string): CustomerInfo | null {
  const match = text.match(CUSTOMER_INFO_REGEX);
  if (!match) return null;
  try {
    const parsed = JSON.parse(match[1]);
    if (parsed.firstName || parsed.lastName) return parsed as CustomerInfo;
    return null;
  } catch {
    return null;
  }
...
export default function CustomerChatWidget({ installerId, installerSlug, installerContext, onAddUnits, skipWelcome, onCustomerInfo }: CustomerChatWidgetProps) {
  ...
  const onCustomerInfoRef = useRef(onCustomerInfo);
  onCustomerInfoRef.current = onCustomerInfo;

  // Check if voice is supported (SpeechRecognition available)
  ...
  // Core send function — returns the full AI response text (used by voice mode)
  const sendMessage = useCallback(async (text: string): Promise<string> => {
    ...
    try {
      ...
      if (reader) {
        ...
        // Parse customerInfo block after streaming completes
        const customerInfo = parseCustomerInfo(fullContent);
        if (customerInfo) {
          onCustomerInfoRef.current?.(customerInfo);
        }
      }
    } catch {
        ...
        <div className="fixed bottom-0 right-0 z-50 flex flex-col sm:bottom-5 sm:right-5 sm:rounded-2xl w-full sm:w-[400px] h-[85vh] sm:h-[540px] bg-slate-900 border border-slate-700 shadow-2xl shadow-black/50 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 bg-slate-800 border-b border-slate-700 shrink-0">
            <div className="flex items-center gap-2">
              ...
              {voiceMode && (
                <span className="rounded-full bg-green-500/20 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-green-400">Voice</span>
              )}
            </div>
            <button
              onClick={() => { setIsOpen(false); setVoiceMode(false); }}
